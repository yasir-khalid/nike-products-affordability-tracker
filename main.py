...
def check_affordability(current_price, my_budget):
    ...
    if current_price <= my_budget:
        ...
        logger.info(f"Out of your affordability range by £{round(current_price - my_budget, 2)}")
        return False
...
def pipeline(nike_url, my_budget = 100):
    ...
    if check_affordability(current_price = price, my_budget = config.MY_BUDGET):
        post_to_slack(f"{merchandise} priced: £ {price} within your affordability range")
    else:
        post_to_slack(f"{merchandise} priced: £ {price} outside of your affordability range")
